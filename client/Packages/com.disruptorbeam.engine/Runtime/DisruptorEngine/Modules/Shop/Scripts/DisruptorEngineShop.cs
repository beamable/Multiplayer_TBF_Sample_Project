...
using Beamable.Modules.Shop.Defaults;
...
{
   [HelpURL(ContentConstants.URL_FEATURE_STORE_FLOW)]
   public class DisruptorEngineShop : MonoBehaviour
   {
      ...
      async void SetupStoreSubscriptions()
      {
         ...
         for (var i = 0 ; i < configuration.Stores.Count; i ++)
         {
            ...
            var subscription = de.Commerce.Subscribe(store.Id, (data) =>
            {
