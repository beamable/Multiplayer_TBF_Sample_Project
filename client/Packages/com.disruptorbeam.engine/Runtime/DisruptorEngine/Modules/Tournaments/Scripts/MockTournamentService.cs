...
namespace Beamable.Modules.Tournaments
{
   public class MockTournamentService : ITournamentService
   {
      ...
      public Promise<TournamentStandingsResponse> GetGlobalStandings(string tournamentId, int cycle = 0, int @from = -1, int max = -1, int focus = -1)
      {
         return GetStandings(tournamentId, cycle);
      }

      public Promise<TournamentStandingsResponse> GetStandings(string tournamentId, int cycle = 0, int @from = -1, int max = -1, int focus = -1)
      {
         if (!string.Equals(tournamentId, Info.tournamentId)) throw new Exception("Invalid tournament id for mock");
