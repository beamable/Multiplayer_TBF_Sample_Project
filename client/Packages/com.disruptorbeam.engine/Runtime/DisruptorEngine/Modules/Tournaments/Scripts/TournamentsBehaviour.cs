...
namespace Beamable.Modules.Tournaments
{
    public class TournamentsBehaviour : MonoBehaviour
    {
        ...
        public Promise<TournamentInfo> ResolveTournamentInfo()
        {
            ...
            {
                return Tournament.Resolve().FlatMap(content => de.Tournaments.GetTournamentInfo(content.Id));
            });
        ...
        Promise<TournamentBundle> GetAllBasicData()
        {
            ...
            return API.Instance.FlatMap(de =>
                Tournament.Resolve()
                    ...
                    .FlatMap(content =>
                    de.Tournaments.GetTournamentInfo(content.Id)
                        .FlatMap(info => de.Tournaments.JoinTournament(info.tournamentId)
                            .Map(status => new TournamentBundle
                            {
                                Content = content,
                                Service = de.Tournaments,
                                TournamentInfo = info,
                                Status = status
                            })
                        )
