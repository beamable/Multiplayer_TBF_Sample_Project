...
namespace Beamable.Tests.Content.ClientManifest
{
   public class ParseCSVTests
   {
      ...
      {
         var manifest = Beamable.Content.ClientManifest.ParseCSV("");
      ...
      {
         var manifest = Beamable.Content.ClientManifest.ParseCSV(null);
         Assert.NotNull(manifest);
         Assert.AreEqual(0, manifest.entries.Count);
      }
      ...
      {
         var manifest = Beamable.Content.ClientManifest.ParseCSV("a0,b0,c0,d0\na1,b1,c1,d1");
         Assert.NotNull(manifest);
      ...
      {
         var manifest = Beamable.Content.ClientManifest.ParseCSV("a0,b0,c0,d0,e1;ee1\na1,b1,c1,d1,e2");
         Assert.NotNull(manifest);
         Assert.AreEqual(2, manifest.entries.Count);
