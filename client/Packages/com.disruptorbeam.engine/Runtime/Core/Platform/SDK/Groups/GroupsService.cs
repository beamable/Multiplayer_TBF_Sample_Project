...
namespace Beamable.Platform.SDK.Groups
{
   public class GroupsService
   {
      private PlatformService _platform;
      private PlatformRequester _requester;
      public GroupsService (PlatformService platform, PlatformRequester requester)
      {
         _platform = platform;
      ...
      public Promise<GroupUser> GetUser (long gamerTag) {
         return _requester.Request<GroupUser>(
            Method.GET,
      ...
      public Promise<Group> GetGroup (long groupId) {
         return _requester.Request<Group>(
            Method.GET,
      ...
      public Promise<GroupMembershipResponse> LeaveGroup (long group) {
          return _requester.Request<GroupMembershipResponse>(
            Method.DELETE,
            String.Format("/object/group-users/{0}/join", _platform.User.id),
      ...
      public Promise<GroupSearchResponse> GetRecommendations () {
         return _requester.Request<GroupSearchResponse>(
            Method.GET,
            String.Format("/object/group-users/{0}/recommended", _platform.User.id)
         );
      }
      ...
      )
      {
         ...
         return _requester.Request<GroupSearchResponse>(
            Method.GET,
            String.Format("/object/group-users/{0}/search?{1}", _platform.User.id, args)
         );
      }

      public Promise<GroupCreateResponse> CreateGroup (GroupCreateRequest request) {
         return _requester.Request<GroupCreateResponse>(
            Method.POST,
            String.Format("/object/group-users/{0}/group", _platform.User.id),
            request
         );
      }
      ...
      public Promise<AvailabilityResponse> CheckAvailability (string name, string tag) {
         ...
         return _requester.Request<AvailabilityResponse>(
            Method.GET,
            String.Format("/object/group-users/{0}/availability?{1}", _platform.User.id, query)
         );
      }
      ...
      private string AddQuery(string query, string key, string value)
      {
         if (query.Length == 0)
         {
            return key + "=" + value;
