...
namespace Beamable.Api.Chat
{
   ...
   public abstract class Room
   {
      ...
      private void MessageReceivedWithBlockFilter(Message message)
      {
         ...
         platform.Social.Get().Then(socialList =>
         {
            if (!socialList.IsBlocked(message.gamerTag))
            {
               AddMessageAndInvokeCallback(message);
            }
            else
            {
               ChatLogger.LogFormat("Hiding message from blocked player {0}", message.gamerTag);
            }
         });
      }
   ...
   [Serializable]
   public struct Message : JsonSerializable.ISerializable
   {
