...
namespace Beamable.Platform.SDK.Mail
{
   public class MailService : PlatformSubscribable<MailQueryResponse, MailQueryResponse>
   {
      public MailService (PlatformService platform, PlatformRequester requester) : base(platform, requester,  "mail")
      {
      }

      public Promise<SearchMailResponse> SearchMail(SearchMailRequest request)
      {
         var url = $"/object/mail/{platform.User.id}/search";

         return requester.RequestJson<SearchMailResponse>(Method.POST, url, request);
      }
      ...
      public Promise<EmptyResponse> Update (MailUpdateRequest updates) {
         return requester.Request<EmptyResponse>(
            Method.PUT,
            $"/object/mail/{platform.User.id}/bulk",
            updates
         );
      }
