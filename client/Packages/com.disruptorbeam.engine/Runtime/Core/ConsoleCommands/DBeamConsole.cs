...
namespace Beamable.ConsoleCommands
{
    ...
    public delegate void OnCommandRegistered(DBeamConsoleCommandAttribute command, ConsoleCommandCallback callback);

    public class DBeamConsole
    {
        ...
        private static DBeamConsole commandInstance;
        ...
        public void LoadCommands()
        {
            ...
            foreach (var assembly in assemblies)
            {
                ...
                {
                    if (!type.IsClass || type.GetCustomAttribute<DBeamConsoleCommandProviderAttribute>(false) == null)
                    {
                        continue;
                    }
                ...
                Func<MethodInfo, ConsoleCommandCallback> callbackCreator)
            {
                foreach (var method in methods)
                {
                    ...
                    var attribute = method.GetCustomAttribute<DBeamConsoleCommandAttribute>();
                    if (attribute == null)
                    {
                        continue;
                    }
        ...
        public string Origin(string command)
        {
            ...
            return $"{command} not found in DBeamCommandAttribute registrations";
        }
    }
}
