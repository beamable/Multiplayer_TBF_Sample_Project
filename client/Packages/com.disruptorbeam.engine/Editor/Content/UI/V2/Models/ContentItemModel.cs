   ...
   public class ContentItemDescriptor
   {
      ...
      public ContentModificationStatus Status
      {
         get
         {
            ...
            if (_localContent != null && _serverData != null)
            {
               return (ContentIO.ComputeChecksum(_localContent).Equals(_serverData.Checksum) && _localContent.Tags.SequenceEqual(_serverData.Tags))
      ...
      public string AssetPathShort =>
         string.IsNullOrEmpty(AssetPath)
            ? ""
            : Path.GetDirectoryName(AssetPath)?.Substring(ContentConstants.DATA_DIR.Length + 1) ?? "";
      ...
      private void ContentObject_OnEditorValidate()
      {
         ...
         if (!string.Equals(_localChecksum, currChecksum) || !_localContent.Tags.SequenceEqual(LocalTags))
         {
            _allTags = CollectAllTags();
