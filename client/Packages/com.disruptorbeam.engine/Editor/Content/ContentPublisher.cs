...
namespace Beamable.Editor.Content
{
   public class ContentPublisher
   {
      ...
      {
         return _io.FetchManifest().Map(serverManifest =>
         {
      ...
      public Promise<Unit> ClearManifest()
      {
         ...
         return _requester.RequestJson<ContentManifest>(Method.POST, "/basic/content/manifest", manifest)
      ...
      public Promise<Unit> Publish(ContentPublishSet publishSet, Action<PublishProgress> progressCallback)
      {
         var operation =
            PushEachPieceOfContentToComet(publishSet, progressCallback)
               .FlatMap(referenceSet => PushManifestToComet(referenceSet.Values.ToList()))
               .Then(_ =>
               {
